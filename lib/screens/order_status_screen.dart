...
class _OrderStatusScreenState extends State<OrderStatusScreen> {
  ...
  Future<void> _loadOrderDetails() async {
    try {
      ...
      if (order == null && mounted) {
        debugPrint('?? Ordem não encontrada no OrderService, tentando OrderProvider...');
        try {
          ...
          debugPrint('?? OrderProvider também falhou: $providerError');
        }
      }
                  ...
                  for (final barcode in barcodes) {
                    ...
                    if (code != null && code.isNotEmpty) {
                      ...
                      if (cleaned.length > 10) {
                        scannedCode = cleaned;
                        debugPrint('?? Código não reconhecido, aceitando: $scannedCode');
                        Navigator.pop(ctx);
                        return;
                      }
                    }
  ...
  Future<void> _submitDispute(String reason, String description) async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('?? Disputa aberta com sucesso! O suporte foi notificado e irá analisar o caso.'),
  ...
  Future<void> _handleConfirmPayment() async {
    ...
    try {
      ...
      if (!updateSuccess) {
        debugPrint('?? Falha ao atualizar status para completed');
      } else {
