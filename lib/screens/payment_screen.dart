    ...
    Future<void> _showLightningInvoiceDialog({
      ...
      eventSub = breezProvider.sdk?.addEventListener().listen((event) {
        ...
        if (dialogClosed || isProcessingPayment) {
          debugPrint('?? Dialog fechado ou já processando, ignorando evento');
          return;
        }
        ...
        builder: (ctx) {
          ...
          return WillPopScope(
            onWillPop: () async {
              ...
              debugPrint('?? Dialog fechado pelo usuário');
  ...
  Future<void> _createPayment({
    ...
    try {
      ...
      if (mounted) {
        ...
        debugPrint('?? Widget não montado, não pode resetar _isProcessing');
      }
    }
  }
