...
/// Servi�o para rastrear taxas da plataforma
/// 
/// MODO ATUAL: TRACKING ONLY
/// - Taxas v�o 100% para provedores
/// - Este servi�o apenas REGISTRA as taxas para an�lise futura
...
class PlatformFeeService {
  ...
  /// Inicializa o servi�o carregando ordens j� pagas do storage
  static Future<void> initialize() async {
    final prefs = await SharedPreferences.getInstance();
    final paidIds = prefs.getStringList(_paidOrderIdsKey) ?? [];
    _paidOrderIds.clear();
    _paidOrderIds.addAll(paidIds);
    debugPrint('?? PlatformFeeService inicializado com ${_paidOrderIds.length} ordens j� pagas');
  }
  ...
  /// Registra uma taxa de transa��o (TRACKING ONLY)
  /// Chamado quando um pagamento � confirmado
  /// A taxa � registrada mas N�O cobrada do provedor
  ...
  // IMPORTANTE: Registro de ordens que j� tiveram a taxa paga para evitar duplica��o
  static final Set<String> _paidOrderIds = {};

  /// Configura o callback de pagamento (chamar na inicializa��o do app)
  ...
  static Future<bool> sendPlatformFee({
    ...
    // VERIFICA��O CR�TICA: Evitar pagamento duplicado
    if (_paidOrderIds.contains(orderId)) {
      debugPrint('?? Taxa j� foi paga para ordem ${orderId.length > 8 ? orderId.substring(0, 8) : orderId} - ignorando');
    ...
    if (AppConfig.platformLightningAddress.isEmpty) {
      debugPrint('?? platformLightningAddress n�o configurado!');
      return false;
    }
    ...
    if (_payInvoiceCallback == null) {
      debugPrint('? ERRO: Callback de pagamento n�o configurado!');
      debugPrint('   Certifique-se de chamar PlatformFeeService.setPaymentCallback() na inicializa��o');
      return false;
    }
    ...
    try {
      ...
      // Detectar tipo de endere�o Lightning
      if (platformAddress.contains('@')) {
        ...
        if (payResult != null && payResult['success'] == true) {
          // CR�TICO: Registrar que esta ordem j� teve taxa paga (persistido)
        ...
        debugPrint('?? BOLT12 Offer detectado - n�o suportado ainda');
        return false;
      ...
      debugPrint('?? Tipo de endere�o n�o reconhecido: $platformAddress');
      return false;
